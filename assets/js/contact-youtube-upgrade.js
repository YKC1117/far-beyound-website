(function(){
  ...
  function addStyle(){
    ...
    style.textContent=`
      ...
      .quick-contact-item[data-contact-phone].is-open .quick-phone-panel,
      .quick-contact-item[data-contact-phone][data-click-open="1"] .quick-phone-panel{opacity:1!important;visibility:visible!important;transform:none!important;pointer-events:auto!important}
  ...
  function prepareDesktopPhone(){
    ...
    if(!btn) return;
    if(!panel){panel=document.createElement('div');panel.className='quick-phone-panel';item.appendChild(panel)}
    ...
    if(!item.dataset.clickOpen)btn.setAttribute('aria-expanded','false');
    item.dataset.desktopPhoneReady='1';
  }

  function setDesktopPhone(item,open){
    const btn=item?.querySelector('.quick-contact-btn');
  ...
  function bindDesktopPhoneCapture(){
    if(window.__fbDesktopPhoneCaptureBound)return;
    window.__fbDesktopPhoneCaptureBound=true;
    document.addEventListener('click',e=>{
      const btn=e.target.closest?.('.quick-contact-item[data-contact-phone] .quick-contact-btn');
      if(btn){
        const item=btn.closest('.quick-contact-item[data-contact-phone]');
        e.preventDefault();
        e.stopImmediatePropagation();
        const open=item.dataset.clickOpen!=='1';
        document.querySelectorAll('.quick-contact-item[data-contact-phone]').forEach(x=>setDesktopPhone(x,false));
        if(open)setDesktopPhone(item,true);
        return;
      }
      const inside=e.target.closest?.('.quick-contact-item[data-contact-phone]');
      if(!inside)document.querySelectorAll('.quick-contact-item[data-contact-phone]').forEach(x=>setDesktopPhone(x,false));
    },true);
    document.addEventListener('keydown',e=>{
      if(e.key==='Escape')document.querySelectorAll('.quick-contact-item[data-contact-phone]').forEach(x=>setDesktopPhone(x,false));
    });
  }
  ...
  function upgradeMobilePhone(){
    ...
    btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const open=!panel.classList.contains('is-open');close();if(open){panel.classList.add('is-open');btn.setAttribute('aria-expanded','true')}});
  ...
  function run(){addStyle();prepareDesktopPhone();bindDesktopPhoneCapture();upgradeMobilePhone();addYoutubeFooter()}
  let queued=false;
  const scheduleRun=()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;run()})};
  document.addEventListener('DOMContentLoaded',()=>{run();setTimeout(run,120);setTimeout(run,600);const observer=new MutationObserver(scheduleRun);observer.observe(document.body,{childList:true,subtree:true})});

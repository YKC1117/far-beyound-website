...
function patchCredibility(c){const strip=document.querySelector('.v5-credibility');if(strip)strip.style.display=c.showCredibility?'block':'none'}function selected(c){const map=productMap();const ids=(c.productIds||DEFAULT.productIds).filter(id=>map.has(id)&&map.get(id)?.published!==false);return ids.map(id=>map.get(id)).filter(Boolean)}function tile(p,kind){const src=photo(p),publicId=window.FBPublicProductId?FBPublicProductId(p.id):p.id,priority=kind==='main'?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"',img=src?`<img src="${esc(src)}" alt="${esc(p.name||'產品')}" ${priority} decoding="async">`:'<div class="home-hero-fallback">PRODUCT</div>';if(kind==='main')return `<a class="v2-stage-main hero-rotate-card is-entering" href="product.html?id=${encodeURIComponent(publicId)}"><span class="v2-stage-label">${esc((p.type||'PRODUCT').toUpperCase())}</span>${img}<span class="v2-stage-caption"><small>${esc(p.subtitle||p.type||'產品')}</small><b>${esc(p.name||'產品')}</b></span></a>`;return `<a class="hero-rotate-side is-entering" href="product.html?id=${encodeURIComponent(publicId)}">${img}<span><small>${esc(p.brand||'')}</small>${esc(p.name||'產品')}</span></a>`}

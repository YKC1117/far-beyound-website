(()=>{
  ...
  async function authFetch(path,{method='GET',body,token}={}){
    const s=token?null:session(),access=token||s?.access_token;if(!access)throw new Error('no_session');
    const r=await fetch(`${URL}/auth/v1/${path}`,{method,headers:{apikey:KEY,Authorization:`Bearer ${access}`,'Content-Type':'application/json'},body:body===undefined?undefined:JSON.stringify(body),cache:'no-store'});
    const b=await r.json().catch(()=>({}));if(!r.ok){const e=new Error(b.error_code||b.code||b.msg||b.message||'auth_request_failed');e.detail=b;throw e}return b
  }
  ...
  async function login(email,password){
    ...
    const p={ok:true,user:b.user||{},permissions:b.permissions||{},aal:assurance({...s}),mfa_required:true};
  ...
  async function profile(token){
    ...
    if(!r.ok)throw new Error(b.error||'session_failed');return {...b,aal:assurance({access_token:token}),mfa_required:true}
  }

  async function current(){let s=session();if(!s?.access_token)return null;try{s=await refreshSession(s,true)||s;return await profile(s.access_token)}catch(e){if(['inactive','unauthorized'].includes(e.message))clearLocal('session_expired');return null}}

  async function setPassword(password){
    let s=session();if(!s?.access_token)throw new Error('no_session');const issues=passwordIssues(password);if(issues.length){const e=new Error('password_policy');e.issues=issues;throw e}
    ...
    if(!r.ok)throw new Error('password_update_failed');sessionStorage.removeItem(INVITE);const p=await profile(s.access_token);notify({state:'login',profile:p});return p
  }

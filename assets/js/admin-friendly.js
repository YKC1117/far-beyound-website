...
function ensure(){
  ...
  $('#adminFriendlySync').addEventListener('click',async()=>{
    ...
    try{await window.FBCloudSync?.pull?.()}finally{b.disabled=false;b.textContent='重新讀取最新資料'}
  });
}
...
function readCloudLabel(){const t=$('#adminCloudState')?.textContent||'';if(!t)return;if(t.includes('正在上傳'))setState('saving','正在把修改發布到共用網站，請稍候。');else if(t.includes('同步中'))setState('syncing','正在讀取共用網站最新資料。');else if(t.includes('上傳失敗'))setState('error','發布失敗，修改仍留在目前瀏覽器，可以重新儲存再試一次。');else if(t.includes('讀取失敗'))setState('error','讀取共用資料失敗，請確認網路後再試一次。');else if(t.includes('未發布'))setState('error','這次修改沒有發布到共用網站。');else if(t.includes('已同步')||t.includes('已載入最新版本'))setState('ready','目前已讀取共用網站最新資料。')}
function watchCloud(){let watched=null;const bind=()=>{const el=$('#adminCloudState');if(!el||el===watched)return;watched=el;new MutationObserver(readCloudLabel).observe(el,{childList:true,characterData:true,subtree:true});readCloudLabel()};bind();new MutationObserver(bind).observe(document.body,{childList:true,subtree:true})}
...
function init(){if(document.body.dataset.page!=='admin')return;style();ensure();watchCloud();document.addEventListener('input',markDirty,true);document.addEventListener('change',markDirty,true);

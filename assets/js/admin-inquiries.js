(()=>{
  ...
  let lastRows=[],legacyPending=null;

  function legacyPassword(){
    if(legacyPending)return legacyPending;
    legacyPending=new Promise(resolve=>{
      let root=document.getElementById('inquiryLegacyAuth');
      if(!root){
        const st=document.createElement('style');
        st.textContent='.inquiry-legacy-auth{position:fixed;inset:0;z-index:10070;display:none;place-items:center;padding:18px;background:rgba(7,24,45,.62);backdrop-filter:blur(5px)}.inquiry-legacy-auth.open{display:grid}.inquiry-legacy-card{width:min(420px,100%);padding:20px;border-radius:16px;background:#fff;box-shadow:0 26px 70px rgba(0,0,0,.28)}.inquiry-legacy-card h3{margin:0 0 6px}.inquiry-legacy-card p{margin:0 0 14px;color:#6c808c;font-size:12px;line-height:1.6}.inquiry-legacy-card input{box-sizing:border-box;width:100%;min-height:44px;padding:10px 12px;border:1px solid #cfdde3;border-radius:10px;font-size:16px}.inquiry-legacy-actions{display:flex;gap:8px;margin-top:12px}.inquiry-legacy-actions .btn{flex:1}';
      ...
      const form=root.querySelector('form'),input=root.querySelector('input'),cancel=root.querySelector('button[type="button"]');
  ...
  function contactHtml(x){
    const parts=[];
    const phone=String(x.phone||'').trim(),mobile=String(x.mobile||'').trim(),email=String(x.email||'').trim();
    if(phone){
  ...
  function render(){
    const body=document.getElementById('inquiryRows');
    if(!body)return;
    const q=(document.getElementById('inquirySearch')?.value||'').trim().toLowerCase(),state=document.getElementById('inquiryEmailFilter')?.value||'';
    ...
    const count=document.getElementById('inquiryVisibleCount');
    if(count)count.textContent=`顯示 ${rows.length}／${lastRows.length} 筆`;
    if(!rows.length){
      body.innerHTML=`<tr><td colspan="6">${lastRows.length?'沒有符合目前搜尋／篩選條件的詢問。':'目前還沒有網站詢問紀錄。'}</td></tr>`;
      return;
    }
    body.innerHTML=rows.map(x=>`<tr><td>${esc(fmt(x.created_at))}</td><td><b>${esc(x.company)}</b><br><span>${esc(x.department||'')} ${esc(x.contact_name||'')}${x.job_title?'／'+esc(x.job_title):''}</span></td><td>${esc(x.subject||'一般詢問')}${x.budget?'<br><span>'+esc(x.budget)+'</span>':''}</td><td>${contactHtml(x)}</td><td><b>${esc(statusLabel(x.email_status))}</b></td><td style="min-width:220px;white-space:normal">${esc(x.message||'—')}</td></tr>`).join('');
  }
  ...
  async function load(){
    ...
    try{
      ...
      const r=await fetch(ENDPOINT+'?limit=100',{headers,cache:'no-store'}),payload=await r.json().catch(()=>null);
      if(!r.ok){const err=new Error(payload?.error||'read');throw err}
      ...
      lastRows=[];
      const msg=e.message==='personal_login_required'?'已啟用個人後台帳號，請先登入並確認具備「網站詢問」權限。':e.message==='forbidden'?'目前登入帳號沒有「網站詢問」權限。':e.message==='unauthorized'?'驗證失敗，未讀取任何客戶資料。':'詢問資料讀取失敗，請稍後再試。';

(function(){
  ...
  function asciiSlug(v=''){
    return String(v).normalize('NFKD').toLowerCase().replace(/&/g,' and ').replace(/\+/g,' plus ').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');
  }
  function normalizeModelSlug(v=''){
    const input=String(v).split('-').filter(Boolean),out=[];
    input.forEach(token=>{
      if(out.length&&token===out[out.length-1])return;
      if(out.length&&/^\d+$/.test(token)){
        const m=out[out.length-1].match(/^([a-z]+)(\d+)$/);
  ...
  function legacyPath(p){try{return new URL(String(p.legacyUrl||''),location.href).pathname}catch{return''}}
  function fastechKeyword(p){
    ...
    const checks=[
      [/全樹脂碳帶/,'resin-ribbon'],[/半[臘蠟]半樹脂碳帶/,'wax-resin-ribbon'],[/全[臘蠟]碳帶/,'wax-ribbon'],[/彩色碳帶/,'color-ribbon'],
      [/銅版標籤紙/,'coated-label'],[/染色標籤紙/,'color-label'],[/反銀龍|消銀龍/,'silver-label'],[/珠光紙/,'pearl-label'],[/白色特多龍/,'white-polyester-label'],
      [/熱感紙|熱感貼紙/,'thermal-label'],[/透明麗龍|透明特多龍/,'clear-label'],[/紙卡|吊牌/,'hang-tag'],[/hifi/i,'hifi-label'],[/易碎紙/,'fragile-label'],[/防偽貼紙/,'security-label'],
      [/外掛紙捲架|紙捲架/,'roll-holder'],[/列印頭|印字頭|printhead/i,'printhead'],[/滾軸/,'platen-roller'],[/皮帶/,'drive-belt'],[/擴充卡/,'expansion-card'],
      [/標籤剝離器.*回捲器|剝離器.*回捲器/,'peeler-rewinder'],[/標籤剝離.*裁刀|剝離.*裁刀/,'peeler-cutter'],[/切紙器|裁刀|標籤裁刀/,'cutter'],
      [/rfid模組|抗金屬模組/i,'rfid-module'],[/馬達/,'motor'],[/碳帶軸心/,'ribbon-spindle'],[/變壓器/,'power-adapter'],[/齒輪/,'gear']
    ];
    const hit=checks.find(([re])=>re.test(text));if(hit)return hit[1];
  ...
  function publicSlugBase(p){
    const verified=VERIFIED_PATH_IDS[legacyPath(p)];if(verified)return verified;
    const brand=asciiSlug(p.brand),name=asciiSlug(p.name),family=asciiSlug(p.family),category=CATEGORY_SLUG[p.category]||asciiSlug(p.category)||'product';
    if(!brand)return normalizeModelSlug(`fastech-${fastechKeyword(p)}`);
    if(p.category==='parts')return normalizeModelSlug(`${brand}-${fastechKeyword(p)}`);
    let base=name;
    ...
    if(base===brand)base=`${brand}-${fastechKeyword(p)}`;
    return normalizeModelSlug(base.replace(/-+/g,'-').replace(/^-+|-+$/g,''))||`product-${(p.legacyOrder??0)+1}`;
  }
  function assignPublicSlugs(items){
    ...
    items.forEach(p=>{
      ...
      if(used.has(slug)){
        ...
        slug=normalizeModelSlug(`${base}-${tail}`);
      }
      const root=slug;let n=2;while(used.has(slug))slug=`${root}-${n++}`;
      p.slug=slug;used.add(slug);
      if(p.legacyUrl){if(!p.sourceId)p.sourceId=p.id;p.id=slug;}
    });
  }
  function categoryIndex(d,id){const n=(d.categories||[]).findIndex(c=>c.id===id);return n<0?999:n}
  function brandIndex(cat,brand){const order=window.FBLegacyCatalog?.brandOrder?.[cat]||[];const n=order.indexOf(brand); return n<0?999:n}
  function enrichOfficial(catalogProducts,existing){
    ...
    return catalogProducts.map(source=>{
      ...
      if(idx<0)return p;
      used.add(idx);const curated=existing[idx];
      return {...p,id:curated.id||p.id,sourceId:p.id,curatedId:curated.id||'',name:curated.name||p.name,subtitle:curated.subtitle||p.subtitle,family:curated.family||p.family,type:curated.type||p.type,status:curated.status||p.status,device:curated.device||p.device,intro:curated.intro||p.intro,highlights:curated.highlights?.length?curated.highlights:p.highlights,specs:curated.specs?.length?curated.specs:p.specs,files:curated.files?.length?curated.files:p.files,featured:!!curated.featured,image:p.image||curated.image||'',category:p.category,brand:p.brand,brandOrder:p.brandOrder,legacyOrder:p.legacyOrder,legacyUrl:p.legacyUrl};
    });
  }
  FBStore.getData=function(){
    const d=rawGet(),catalog=window.FBLegacyCatalog;if(!catalog||!Array.isArray(catalog.products))return d;

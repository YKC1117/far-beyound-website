(function(){
  ...
  function upgradeBrandPortfolio(){
    const note=document.querySelector('.brand-portfolio-head>p');
    if(note)note.textContent='多品牌設備選型、耗材供應與技術服務';
  ...
  function orderIndex(name){
    let normalized=name;
    if(normalized.startsWith('Honeywell'))normalized='Honeywell';
    if(normalized.startsWith('遠端連線'))normalized='遠端連線';
    const index=DOWNLOAD_ORDER.indexOf(normalized);
  ...
  function fixDownloadNavigation(){
    ...
    document.querySelectorAll('.mobile-nav-group').forEach(group=>{
      ...
      const box=group.querySelector('.mobile-nav-sub');
      if(!box)return;
  ...
  // 只換皮膚：不再刪除首頁原有區塊，也不再截斷產品與消息資料。
  function preserveHomeContent(){
  ...
  function run(){
    applyBrandIdentity();
    upgradeProductMega();
    fixDownloadNavigation();
    preserveHomeContent();
    upgradeBrandPortfolio();
    cleanFooter();
  }

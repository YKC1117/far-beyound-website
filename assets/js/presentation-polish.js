(function(){
  ...
  function watchPublicCopy(){
    ...
    const ob=new MutationObserver(()=>queueMicrotask(sanitizePublicText));
  ...
  function run(){cleanCopy();watchPublicCopy();addBackTop();contactNote()}
  document.addEventListener('DOMContentLoaded',()=>{run();setTimeout(run,100);setTimeout(run,700);setTimeout(sanitizePublicText,1800)});
  if(document.readyState!=='loading'){setTimeout(run,0);setTimeout(run,700);setTimeout(sanitizePublicText,1800)}
})();

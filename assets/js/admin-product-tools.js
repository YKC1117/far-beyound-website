(function(){
  ...
  function reloadAfterPublish(){
    ...
    setTimeout(()=>{if(!done&&$('#adminCloudState')?.textContent?.includes('已上傳'))go()},1500);
  }
  ...
  function toggleFeatured(p){
    ...
    if(!confirm(`${next?'加入':'移出'}首頁精選？\n\n${item.brand||''} ${item.name||item.id}\n\n儲存時仍會要求輸入發布密碼。`))return;
    item.featured=next;saveAndRefresh(d);
  }
  function duplicateProduct(p){
    const d=data(),item=d.products.find(x=>String(x.id)===String(p.id));if(!item)return;
    if(!confirm(`確定複製這項產品？\n\n${item.brand||''} ${item.name||item.id}\n\n系統會建立一份副本，原產品不會被修改。`))return;
    const copy=JSON.parse(JSON.stringify(item));copy.id=uniqueCopyId(item.id,d.products);copy.name=(item.name||item.id)+'（副本）';copy.featured=false;
  ...
  function decorate(){
    ...
    [...body.rows].forEach(row=>{
      ...
      const status=row.cells?.[3];if(status&&!status.querySelector('.pm-pill'))status.insertAdjacentHTML('beforeend',`<div><span class="pm-pill ${p.featured?'on':''}">${p.featured?'首頁精選':'一般產品'}</span></div>`);
      const cell=row.cells?.[row.cells.length-1];if(!cell||cell.querySelector('.pm-actions'))return;
      const box=document.createElement('span');box.className='pm-actions';box.innerHTML=`<a class="icon-text" href="product.html?id=${encodeURIComponent(p.id)}" target="_blank">預覽</a><button type="button" class="icon-text pm-featured">${p.featured?'取消精選':'首頁精選'}</button><button type="button" class="icon-text pm-copy">複製</button>`;cell.insertBefore(box,cell.firstChild);
      $('.pm-featured',box).onclick=e=>{e.stopPropagation();toggleFeatured(p)};$('.pm-copy',box).onclick=e=>{e.stopPropagation();duplicateProduct(p)};
    ...
    if(sum)sum.innerHTML=`<span><b>${d.products.length}</b> 項產品</span><span><b>${d.products.filter(p=>p.featured).length}</b> 項首頁精選</span><small>編輯、精選、複製或刪除，儲存時都會要求發布密碼。</small>`;
  }
  function humanizeForm(){
    ...
    const f=$('#productAdminForm');if(f&&!$('#pmFormHelp',f)){const n=document.createElement('div');n.id='pmFormHelp';n.className='pm-form-help';n.textContent='建議只修改你確定的欄位。按「儲存產品」後會先顯示本次異動，再要求輸入發布密碼。';f.insertBefore(n,f.firstChild)}
  }
  function style(){if($('#pmStyle'))return;const s=document.createElement('style');s.id='pmStyle';s.textContent='.pm-actions{display:inline-flex;gap:6px;align-items:center;margin-right:6px}.pm-actions button{background:none;border:0;padding:0;cursor:pointer}.pm-pill{display:inline-block;margin-top:4px;padding:3px 7px;border-radius:999px;background:#f1f5f7;color:#71838d;font-size:9px;font-weight:900}.pm-pill.on{background:#eaf8f1;color:#23744e}.pm-summary{display:flex;gap:10px;align-items:center;flex-wrap:wrap;padding:10px 18px;border-bottom:1px solid #edf1f3;background:#fafcfd;color:#657985;font-size:10px}.pm-summary span{padding:4px 8px;border:1px solid #e1e9ed;border-radius:8px;background:#fff}.pm-summary b{color:#23485a}.pm-summary small{margin-left:auto}.pm-form-help{margin:0 0 14px;padding:10px 12px;border:1px solid #e1ecef;border-radius:10px;background:#f4f9fb;color:#58717e;font-size:10px;line-height:1.6}';document.head.appendChild(s)}
  function captureDelete(e){
    const btn=e.target.closest?.('#products button');if(!btn||btn.classList.contains('pm-featured')||btn.classList.contains('pm-copy')||!btn.textContent.trim().includes('刪除'))return;
    const row=btn.closest('tr'),p=productFromRow(row);if(!p)return;e.preventDefault();e.stopImmediatePropagation();deleteProduct(p,row);
  }
  function init(){
    if(document.body.dataset.page!=='admin')return;const body=$('#adminProductRows');if(!body)return;style();humanizeForm();fillCategories();decorate();
  ...
  window.addEventListener('farbeyound:datachange',()=>setTimeout(()=>{fillCategories();decorate();apply()},60));
})();
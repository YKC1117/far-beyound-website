...
function fieldKey(el,index){return el.id||el.name||`${el.tagName.toLowerCase()}:${el.type||''}:${index}`}
...
function displayValue(el){
  ...
  if(el.type==='radio')return el.checked?(el.closest('label')?.textContent||el.value):'未選擇';
  if(el.tagName==='SELECT')return el.options[el.selectedIndex]?.textContent||el.value||'（空白）';
  const v=String(el.value||'').trim();if(!v)return '（空白）';return v.length>180?v.slice(0,180)+'…':v;
}
function eligible(form){return $$('input,textarea,select',form).filter(el=>el.type!=='hidden'&&el.type!=='file'&&!el.disabled)}
function snapshot(form){
  const out={};eligible(form).forEach((el,i)=>{out[fieldKey(el,i)]={raw:rawValue(el),display:displayValue(el),label:labelOf(el),type:el.type||el.tagName.toLowerCase()}});return out;
}
function remember(form,force=false){if(!form||(!(form instanceof HTMLFormElement)))return;if(force||!baselines.has(form))baselines.set(form,snapshot(form))}
function changed(form){
  const before=baselines.get(form)||{};const now=snapshot(form);const items=[];
  Object.entries(now).forEach(([key,n])=>{
    const b=before[key];
    if(!b||b.raw!==n.raw)items.push({label:n.label,before:b?.display??'（未設定）',after:n.display});
  });
  return items;
}
...
function open(form,submitter){
  ...
  const items=changed(form),box=$('#adminSaveGuardSummary');
  if(!items.length){box.innerHTML='<div class="admin-save-guard-empty"><b>沒有偵測到修改內容</b><br>目前欄位與開啟時相同，不需要重複儲存。</div>';$('#adminSaveGuardCheck').checked=false;$('#adminSaveGuardCheck').disabled=true;$('#adminSaveGuardConfirm').disabled=true;const modal=$('#adminSaveGuard');modal.classList.add('open');modal.setAttribute('aria-hidden','false');return}
  box.innerHTML=items.map(x=>`<div class="admin-save-guard-row"><b>${esc(x.label)}</b><div class="admin-save-guard-change"><small>原本</small><span>${esc(x.before)}</span><small>修改後</small><strong>${esc(x.after)}</strong></div></div>`).join('');
  $('#adminSaveGuardCheck').disabled=false;$('#adminSaveGuardCheck').checked=false;$('#adminSaveGuardConfirm').disabled=true;
...
function confirmSave(){
  ...
  close();
  setTimeout(()=>{try{if(typeof form.requestSubmit==='function')form.requestSubmit(submitter&&submitter.form===form?submitter:undefined);else form.dispatchEvent(new Event('submit',{bubbles:true,cancelable:true}))}finally{setTimeout(()=>{delete form.dataset.saveGuardConfirmed;remember(form,true)},60)}},0);
}

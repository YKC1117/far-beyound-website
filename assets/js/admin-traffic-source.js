(()=>{
  ...
  async function load(days){const s=window.FBAdminAuth?.session?.();if(!s?.access_token)throw new Error('login_required');const r=await fetch(`${ENDPOINT}?period_days=${days}`,{headers:{Authorization:`Bearer ${s.access_token}`},cache:'no-store'}),b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.error||'read_failed');return b.sources||[]}
  function ensure(){const box=document.getElementById('adminAnalytics');if(!box||document.getElementById('analyticsSourcePanel'))return;const panel=document.createElement('div');panel.id='analyticsSourcePanel';panel.innerHTML='<h3 style="margin:24px 18px 10px">流量來源</h3><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>來源</th><th>本期點閱</th><th>前期</th><th>變化</th></tr></thead><tbody id="analyticsSourceRows"><tr><td colspan="4">請先登入具備流量分析權限的個人帳號。</td></tr></tbody></table></div><div id="analyticsSourceNote" class="admin-usage-note"><b>AI SEO：</b>AI 助理導流只代表可辨識的轉介流量，不代表 AI 提及或引用次數。</div>';const status=document.getElementById('analyticsStatus');status?status.after(panel):box.appendChild(panel);document.getElementById('analyticsPeriod')?.addEventListener('change',()=>{document.getElementById('analyticsSourceRows').innerHTML='<tr><td colspan="4">期間已變更，等待分析資料重新讀取。</td></tr>'});window.addEventListener('farbeyound:adminauth',refresh);setTimeout(refresh,1600)}
